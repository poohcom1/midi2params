...
def parse_arguments():
    ...
    parser.add_argument('--worker', '-w', type=int,
                        default=0, help='Worker ID.')
    parser.add_argument('--total', '-t', type=int, default=1,
                        help='Total number of workers.')
    parser.add_argument('--path', '-p', type=str, default='data/DDSPViolin-clipped2/wav',
                        help='Path to folder; can contain subfolders as long as every file (no matter how deep) is a WAV file.')
...
def extract(fpath, basepath, force=False):
    ...
    # how to get from basepath to the specific file
    relpath = os.path.relpath(pure_fpath, basepath)
    # switch basepath from synths folder to the parameters folder
    basepath = os.path.join(os.path.dirname(
        os.path.split(basepath)[0]), BASE_FOLDER_NAME)
...
if __name__ == '__main__':
    ...
    for idx, file in enumerate(fl):
        print('worker #{}: {} / {}'.format(i, idx, len(fl)), end=' : ')
        print(file, FOLDER_PATH, end='\n')
